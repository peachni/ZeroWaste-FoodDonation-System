...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['food_id'])) {
    ...
    if (!$conn_balqis->connect_error) {
        ...
        // Logic: Decrease Quantity (stock), Increase Qty_Consumed (total given out)
        ...
        if ($conn_balqis->affected_rows > 0) {
            // Auto-update status to Unavailable if stock hits 0
            if ($remainder <= 0) {
                $conn_balqis->query("UPDATE Food_Don_List SET Status = 'Unavailable' WHERE FoodList_ID = '$food_id'");
            }
            $balqis_updated = true;
        } else {
            $conn_balqis->close();
    ...
    if ($balqis_updated) {
        ...
        if ($conn->query($sql_nadia)) {
            ...
            // If Nadia's DB fails, we MUST reverse BOTH columns on Balqis's side
